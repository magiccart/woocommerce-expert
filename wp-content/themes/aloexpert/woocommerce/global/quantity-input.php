...
if ( $max_value && $min_value === $max_value ) {
	...
	<div class="product-qty clearfix">
		<label><?php _e('Quantity:', 'aloexpert') ?></label>
		<div class="quantity clearfix">
			<input type="text" id="qty" class="input-text qty text" step="<?php echo esc_attr( $step ); ?>" min="<?php echo esc_attr( $min_value ); ?>" max="<?php echo esc_attr( 0 < $max_value ? $max_value : '' ); ?>" name="<?php echo esc_attr( $input_name ); ?>" value="<?php echo esc_attr( $input_value ); ?>" title="<?php echo esc_attr_x( 'Qty', 'Product quantity input tooltip', 'woocommerce' ) ?>" size="4" pattern="<?php echo esc_attr( $pattern ); ?>" inputmode="<?php echo esc_attr( $inputmode ); ?>" />
			<button  type="button" class="increase btn-plus">
				<i class="fa fa-plus"></i>
			</button>
			<button  type="button" class="reduced btn-plus">
