 ...
 * @@Modify Date: 2018-05-17 14:39:48
 * @@Function:
 */
 
namespace Alothemes\Core\Block\Template;
...
class AbstractTemplate {
    ...
    protected function getModule(){
        if(!$this->_moduleDir){
            ...
            $this->_moduleDir = ALO_PLUGIN_DIR . DIRECTORY_SEPARATOR . $dirName[0] . DIRECTORY_SEPARATOR . $dirName[1];
        }
    ...
    public function toHtml(){
        ...
        if(!file_exists($fileName)) return $this->addError(printf( esc_html__( "File %s not exist.", 'aloexpert' ), $this->_template ));
    ...
    public function getThemeFile($fileName, $message=true){
        ...
        if(!file_exists($templateName)){
            ...
            if(!file_exists($templateName)){
                if($message) return $this->addError(printf( esc_html__( "File %s not exist.", 'aloexpert' ), $fileName ));
                return;
            }
        }
        return $templateName;
    ...
    {
        // $error = new \WP_Error( 'broke', __( "File $this->_template not exist", 'aloexpert' ) );
