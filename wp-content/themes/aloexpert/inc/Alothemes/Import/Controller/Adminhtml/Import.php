 ...
 * @@Modify Date: 2018-03-09 18:20:26
 * @@Function:
 */
...
class Import extends Action {
	...
	public function subMenu(){
		add_submenu_page($this->_menu_slug, __('Import', 'aloexpert'), __('Import', 'aloexpert'), 'manage_options',
				$this->_menu_slug . '-import' , array($this, 'importAction'));
		add_submenu_page($this->_menu_slug, __('Export', 'aloexpert'), __('Export', 'aloexpert'), 'manage_options',
	...
	public function exportAction(){
		...
		if(isset($_POST['redux'])){
			...
			$posts[] = $_POST['header'];
            $posts = array_merge($posts, $_POST['pages']);
			$posts[] = $_POST['footer'];
            try {
                $this->export_wp_posts($dir, $posts);
                $message = '<div id="message" class="updated"><p>' . __('Export data successfully to path: ', 'aloexpert') . $dir . '</p></div>';
            } catch (Exception $e) {
	...
	public function importAction($withEmpty = true){
		...
		if(isset($_POST['theme'])){
            ...
            if(!file_exists($wp_post)){
                echo __('File not exist:', 'aloexpert') . $wp_post;
                die();
            }
            $wp_option = $dir . '/wp_option.xml';
            if(!file_exists($wp_post)){
                echo __('File not exist:', 'aloexpert') . $wp_option;
                die();
            }
            ;
            ...
            try {
                ...
                $message = '<div id="message" class="updated"><p>' . __('Import data successfully from path: ', 'aloexpert') . $dir . '</p></div>';
        ...
        $label = $themeOptions ? __('-- Select Theme --', 'aloexpert') : __('-- Not found theme --', 'aloexpert');
        if ($withEmpty) {
